...
function New-ProofImage {
    ...
    $graphics.DrawString("Editorial Standards Approval Desk", $eyebrowFont, $accentBrush, 92, 92)
...
New-ProofImage -Path (Join-Path $screenshots "01-overview-proof.png") `
    ...
    -Subtitle "Editorial changes, policy blockers, and approval posture in one publishing-safe control surface." `
    -Bullets @(
        "Editorial changes map to concrete publishing impact.",
        "Blocked policy reviews stay visible before launch windows break.",
        "Approval posture is buyer-readable and operator-safe."
    )

New-ProofImage -Path (Join-Path $screenshots "02-editorial-lane-proof.png") `
    -Title "Editorial lane" `
    -Subtitle "Each draft change shows owner, review risk, and next action." `
    -Bullets @(
        "Editorial changes stay linked to approvals and publish impact.",
        "Owners see the next standards-safe move.",
        "High-risk claim drift surfaces early."
    )

New-ProofImage -Path (Join-Path $screenshots "03-standards-gaps-proof.png") `
    -Title "Standards gaps" `
    -Subtitle "Editorial, legal, and brand blockers stay tied to required evidence." `
    -Bullets @(
...
New-ProofImage -Path (Join-Path $screenshots "04-approval-posture-proof.png") `
    -Title "Approval posture" `
    -Subtitle "Approval packets show completeness score, review window, and go/no-go pressure." `
    -Bullets @(
        "Red packets show immediate publish risk.",
